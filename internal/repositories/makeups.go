...
func (r *makeupRepository) GetMakeupsByIDandDate(userID, date string) ([]*models.Makeup, error) {
	query := `
		SELECT user_id, makeup_date, start_time, reason
		FROM makeups
...
func (r *makeupRepository) CreateMakeup(makeup *models.Makeup) error {
	query := `
		...
		VALUES ($1, $2, $3, $4)
	`
	_, err := r.db.Exec(query, makeup.UserID, makeup.Date, makeup.Time, makeup.Reason)
	if err != nil {
		return err
	}
	return nil
...
func (r *makeupRepository) UpdateMakeup(userID, date, time string, makeup *models.Makeup) error {
	query := `
		UPDATE makeups
		SET makeup_date = $1, start_time = $2, reason = $3

		WHERE user_id = $4 AND makeup_date = $5 AND start_time = $6
	`
	result, err := r.db.Exec(query, date, time, makeup.Reason, userID, date, time)
	if err != nil {
		return err
	}
